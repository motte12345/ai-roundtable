...
- 投稿上限は **300 grapheme cluster**（文字数ではない）。`Intl.Segmenter` で数える。**`granularity:'grapheme'` ではロケール引数は境界に影響しない**（word/sentence のみロケール依存）ので `undefined` でよい。Workers は ES2022 lib に `Intl.Segmenter` 型あり・ランタイムも対応
