...
- **ハッシュタグも同じく facet 必須**。`#AI` と書くだけでは検索対象にならない。`app.bsky.richtext.facet#tag`（`tag` は # 抜きの文字列、byte 範囲は `#tag` 込み）を付ける。複数タグは `base + section` を組み立てながら各 `#tag` の byteStart を `encode(base+section前半).length` で実測すると確実（`indexOf` だと `#AI` が `#AI議論` の部分一致でズレる罠）。実装後は **各 facet 範囲をデコードして指す文字列が一致するか必ず検証**する
...
- **スレッド連結を「直前ターン固定」にすると1回の失敗でスレッド全体が止まる**（2026-06-25 本番で発生：turn4 が一時的に投稿失敗 → turn5+ は parent=turn4 が無くスキップ → カスケード停止）。**parent は「直近で投稿成功したターン」(`getLatestPostedBskyRefBefore`) にする**と、単発失敗から次ターンで自動復帰する（root=turn1 だけは必要）。best-effort 連投は「途中の歯抜けに耐える」設計にしておく
- **失敗の多くは一時的なのでリトライで吸収できる**（turn4 の失敗も直後の手動テストは成功）。`createSessionWithRetry`/`postRecordWithRetry`（線形バックオフ）で再試行。ただし**チャンク単位でリトライ**すること（ターン全体を再試行すると投稿済みチャンクが二重投稿）。注意: `com.atproto.repo.createRecord` に冪等キーが無いので「投稿成功したが応答が届かず throw→再試行」では二重投稿が残る（best-effort 許容）。setTimeout は Workers で利用可・cron の I/O 待ちは CPU 制限に当たらない
...
## 2026-06-24: ハイライト機能が 0/410 で「無言の全滅」していた

### 問題
完了議題 410件すべてで `topics.highlights IS NULL`。リリース以来**一度も生成成功していなかった**のに、best-effort catch（`[highlights] error`）で握り潰され、誰も気づかなかった。SEOタイトル（Gemini）は出ていたのでハイライト経路だけの問題。

### 原因（二重）
実 API で再現して特定：
1. **出力スキーマのキー名不一致**: `SYSTEM_PROMPT` が JSON 形式の具体例を示しておらず、モデルが `{"turn_no":N}` ではなく **`{"turn":N}`** を返す。検証 `validateHighlight` は `turn_no` を見るので全件 `undefined`→null。→ Mistral/Groq どちらでも全滅
2. **TPM 超過で 413**: ハイライト用に**議論全文（実測 ~500字 × 11発言）**を Groq `llama-3.1-8b-instant`（無料枠 **TPM 6000**）に投入 → `HTTP 413 Request too large`。コメントは「~2700 tokens」想定だったが、発言が実測500字（[[同じく Bluesky で発覚した「発言は250字でなく500字」問題]]）で倍増していた

### 解決策
- **プロンプトに出力 JSON の具体例を埋める**（`{"sharpest":{"turn_no":3,"reason":"..."},...}`、キー名厳守）
- **各ターンを 200字に切り詰めて渡す**（`HIGHLIGHT_EXCERPT_LEN`）。ハイライト「選定」にはどのターンが際立つか分かれば十分で全文不要 → トークン半減で TPM クリア
- 防御的に `validateHighlight` で `turn_no ?? turn` の両方を受ける
- temperature 0.5→0.3（スキーマ遵守向上）
- 検証: Groq 8B（最弱モデル）で turn_no=9/5/7・各valid・3ターン相異・総合PASS、usage 3919 tokens（<6000）

### 教訓
- **best-effort で握り潰す機能は「無言で0%」になり得る**。出力が貯まる場所（DB列）を時々 `COUNT(... IS NOT NULL)` で観測する仕組みが無いと、壊れていても気づけない
- **LLM に構造化出力させるならスキーマの具体例をプロンプトに必ず埋める**。「JSON で返す」だけだとキー名・ネストはモデル任せ＝弱いモデルほど外す
- **無料枠は RPD だけでなく TPM（tokens/分）も効く**。Groq 8B は TPM 6000 が細い。1リクエストの中身が大きい用途（全文要約・ハイライト）は入力トークンを削る設計を最初から入れる
- 「発言は実測500字」という1つの実態が、Bluesky 切れ・ハイライト413 と**複数箇所で別々に牙を剥いた**。仕様値（250字）でなく実測値で各経路のキャパを設計する

---

## 2026-06-25: Workers AI 導入で踏んだ罠（this / レスポンス形 / Gemini 3.5 不安定 / タイトル汚染）

### ① `env.AI.run` を detach して呼ぶと壊れる
`const run = env.AI.run; run(model, opts)` のようにメソッドを切り離して呼ぶと `this` を失い、SDK 内部の private field で **`Cannot set properties of undefined (setting '#options')`**。**必ず binding 経由 `env.AI.run(...)` で呼ぶ**（`this` を保つ）。型のユニオン回避でキャストするなら、メソッドを外に出さず `(ai as unknown as {run(...)}).run(...)` の形にする。

### ② 新しめの Workers AI テキストモデルは OpenAI 形式を返す
`@cf/meta/llama-3.3-70b-instruct-fp8-fast` は `{response}` ではなく **`{choices:[{message:{content}}]}`**（OpenAI互換）を返す。旧来の `{response}` だけ読むと空→fallback で気付けない。両対応 `resp.response ?? resp.choices?.[0]?.message?.content ?? ''` にする。プローブ実機で確認必須（モデルごとに違う）。
→ **教訓**: Workers AI は「binding は動く」「モデルは存在する」でも、呼び方とレスポンス形で二重に転ぶ。一時デバッグendpointで生レスポンスを必ず見る。

### ③ Gemini 3.5-flash は無料枠で 503(high demand) 多発
`gemini-3.5-flash` を Optimist に採用したら 503 が頻発し毎回 fallback(groq 8B)に落ちて品質劣化（本番 messages の provider 列で発覚）。**実測（各5回）: `gemini-3.1-flash-lite`=5/5・`gemini-2.5-flash`=3/5・`gemini-3.5-flash`=0/5(全部503)**。唯一安定して使えるのは **flash-lite** だけ（2.5-flash も不安定で、一度差し戻したが再度落ちて気付いた）。新しい flash 系は「応答するか」1回だけでなく「**数回叩いて 503 率を測る**」こと。fallback が静かに劣化を吸収するので **provider 列を時々監視**しないと気付けない（ハイライト0/410 と同じ「無言の劣化」）。安定するまで Optimist/Host とも flash-lite に寄せる（合計~49 RPD で枠は余裕）。

### ④ プロンプトのプレースホルダ波括弧がそのまま出力される
Host の次議題テンプレが `[{ジャンル}]`（波括弧入りプレースホルダ）だったため、モデルが `[{tech}]` を literal 出力 → genre 抽出正規表現 `\[([a-zA-Z_]+)\]` が波括弧で外して **タイトルに `[{tech}]` が109件混入**。修正＝テンプレを `[ジャンル]` に（凡例の `[tech]` と整合）＋パーサも `\[\{?...\}?\]` で防御。**プロンプトの出力例フォーマットはモデルがそのまま真似る**。プレースホルダ記法（`{}`）を最終出力に混ぜない。
