...
`gemini-3.5-flash` を Optimist に採用したら 503 が頻発し毎回 fallback(groq 8B)に落ちて品質劣化（本番 messages の provider 列で発覚）。**`gemini-3.1-flash-lite` は安定、`gemini-2.5-flash` も安定**。新しい flash 系は「応答するか」だけでなく「混雑で 503 にならないか」も数ターン観察してから採用する。fallback が静かに劣化を吸収するので **provider 列を時々監視**しないと気付けない（ハイライト0/410 と同じ「無言の劣化」）。
